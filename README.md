...
The **Quality** workflow's `sanitizers` job runs this command on every push and pull request, gated on `integration-tests` passing first. ThreadSanitizer remains a separate opt-in diagnostic because it cannot be combined with ASan and is not reliable enough on every host for a required pipeline gate.

Coverage artifacts are written to `tests/results/UTs/`. The `unit-tests-coverage` job runs on every push and pull request. It uses the same `gcovr` output generated by `make_UTs_cov.sh` and fails unless both line and branch coverage are 100%; therefore a passing **Coverage** badge reflects the enforced pipeline result, rather than a separately maintained value.
